...
address first;
address last;
address alokasikan(string data_x)
{
  address x = new tnode;
  x->previous = NULL;
  x->next = NULL;
...
int main()
{
  ...
  insertFirst(alokasikan("lima"));
  insertFirst(alokasikan("balon ku"));
  insertFirst(alokasikan("rupa-rupa"));
  cetak();
  cout << "Insert Last 70:" << endl;
  insertLast(alokasikan("hallo"));
  cout << "insert after 100:" << endl;
  insertAfter(first->next->next, alokasikan("emaggitu"));
  cetak();
